...
-- 
-- INSTRUCTIONS:
-- 1. FIRST: Create 4 test users in Supabase Dashboard → Authentication → Users
--    Click "Add user" and create:
--    - Email: inventor@test.com, Password: Test123!@#
--    - Email: startup@test.com, Password: Test123!@#
--    - Email: company@test.com, Password: Test123!@#
--    - Email: investor@test.com, Password: Test123!@#
--
-- 2. THEN: Run this script - it will automatically find their UUIDs
--
-- 3. If you get "No auth users found" error, go back to step 1

-- =========================
-- Get UUIDs from auth.users (automatically)
-- =========================
DO $$
DECLARE
  inventor_id uuid;
...
BEGIN
  ...
  -- Warn about missing users (but don't fail)
  IF inventor_id IS NULL THEN
    RAISE NOTICE 'WARNING: Auth user not found: inventor@test.com. Skipping Inventor sample data. Create this user in Authentication → Users to include it.';
  END IF;
  IF startup_id IS NULL THEN
    RAISE NOTICE 'WARNING: Auth user not found: startup@test.com. Skipping StartUp sample data. Create this user in Authentication → Users to include it.';
  END IF;
  IF company_id IS NULL THEN
    RAISE NOTICE 'WARNING: Auth user not found: company@test.com. Skipping Company sample data. Create this user in Authentication → Users to include it.';
  END IF;
  IF investor_id IS NULL THEN
    RAISE NOTICE 'WARNING: Auth user not found: investor@test.com. Skipping Investor sample data. Create this user in Authentication → Users to include it.';
  END IF;

  -- =========================
  -- Sample Inventor User
  -- =========================
  IF inventor_id IS NOT NULL THEN
  ...
  -- Sample StartUp User
  -- =========================
  IF startup_id IS NOT NULL THEN
    INSERT INTO public.users (id, user_type, full_name, personal_email, telephone, country, city, cover_image_url, photo_url)
  VALUES (
    startup_id,
    'StartUp',
  ...
  RAISE NOTICE 'Sample data insertion completed! Check warnings above for any skipped users.';
END $$;
...
WHERE u.personal_email IN ('inventor@test.com', 'startup@test.com', 'company@test.com', 'investor@test.com')
ORDER BY u.user_type;

SELECT cp.user_id, u.full_name, u.user_type,
  cp.equity_capital_percentage, cp.franchisee_investment, cp.patent_upfront_fee
FROM public.commercial_proposals cp
JOIN public.users u ON cp.user_id = u.id
ORDER BY u.user_type;

SELECT pm.user_id, u.full_name, u.user_type,
  pm.pitch_video_url IS NOT NULL as has_video,
  array_length(pm.photos_urls, 1) as photo_count,
  array_length(pm.pitch_videos_urls, 1) as video_count
FROM public.pitch_materials pm
